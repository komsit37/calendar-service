...
lazy val `calendar-root` = (project in file(".")).
  settings(
    name := "calendar",
    organization := "com.folio",
    moduleName := "calendar", //"thrift-example-root",
    run := {
      (run in `calendar-server` in Compile).evaluated
    }
  ).
  aggregate(`calendar-idl`, `calendar-server`)

lazy val `calendar-server` = (project in file("calendar-server")).
  settings(baseSettings).
  settings(
    name := "calendar-server",
    moduleName := "calendar-server",
    libraryDependencies ++= Seq(
  ...
  dependsOn(`calendar-idl`)

lazy val `calendar-idl` = (project in file("calendar-idl")).
    settings(baseSettings).
    settings(
      name := "calendar-idl",
      moduleName := "calendar-idl",
